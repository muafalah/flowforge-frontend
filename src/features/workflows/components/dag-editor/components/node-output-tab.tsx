...
export function NodeOutputTab({ result, isRunning, onRun }: NodeOutputTabProps) {
  const statusBg =
    ...
    <div className="flex-1 flex flex-col min-h-0">
      <div className="flex-1 overflow-y-auto py-4 space-y-4">
        ...
        {isRunning ? (
          <div className="flex items-center gap-3 rounded-lg border border-amber-500/20 bg-amber-500/10 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-amber-600/70">Executing this node. Please wait.</p>
            </div>
          </div>
        ) : result ? (
          <div className={`flex items-center gap-3 rounded-lg border p-4 ${statusBg}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">Completed in {result.durationMs}ms</p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-3 rounded-lg border bg-muted/30 p-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-muted-foreground">No execution data</p>
              <p className="text-xs text-muted-foreground">Click &quot;Run Node&quot; to test this node.</p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Execution Summary</h4>
          <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-1.5">
                {isRunning ? (
                  <>
                    ...
                    <span className="text-sm text-amber-600 font-medium">Running</span>
                  </>
                ) : result ? (
                  <>
                    <CircleDot className={`size-3.5 ${result.status === "SUCCESS" ? "text-emerald-500" : "text-red-500"}`} />
            ...
            <div className="rounded-md border p-3 space-y-1">
              ...
              <div className="flex items-center gap-1.5">
                <Clock className={`size-3.5 ${result ? "text-foreground/60" : "text-muted-foreground/50"}`} />
        ...
        {/* Output Data */}
        <div className="space-y-2">
          <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Output Data</h4>
            ...
            {isRunning ? (
              <div className="flex items-center justify-center h-[120px]">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-500">Waiting for output...</p>
                </div>
              </div>
            ) : result ? (
              ...
              <div className="flex items-center justify-center h-[120px]">
                <div className="text-center space-y-1">
                  ...
                  <p className="text-xs text-slate-500">Output will appear here after execution</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="px-6 py-4 border-t flex items-center justify-end gap-2">
        <Button onClick={onRun} disabled={isRunning} className="gap-1.5" size="sm">
